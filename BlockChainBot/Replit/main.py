...
def RemoveUnsavedTransactions():
  currT = T.TotalTransactions()
  currB = B.TotalBlocks()
  if currB != currT:
    for i in range(currB+1,currT):
      T.DeleteTransaction(i)
  return
  ...
  if len(text) > 1:
    if text[1] in Transfer: 
      ...
      if new_message.content.lower() in Yes:
        ...
        InfoTuple = MyTransaction.Store()
        #Got the Transaction ID
        temp = "The Transaction has been Stored.\nThe Transaction ID is `"+str(InfoTuple[0])+"`\n The corresponding Code is:-  **`" + str(InfoTuple[1]) + "`**"
        '''
        if ID != 0:
